...
const toggleMenu = () => {
    ...
    // Hide submenu when main menu is toggled
    submenuVisible.value = false;
    // Toggle body overflow to prevent scrolling when the menu is open
    document.body.style.overflow = menuVisible.value ? 'hidden' : 'auto';
};
...
<template>
    ...
    <div role="menu" class="menu" :class="{ active: menuVisible }">
        ...
        <div role="button" class="toggle" @click="toggleMenu">
        ...
        <ul>
            ...
            <li role="menuitem" @click="closeMenu"><router-link to="/office">Office</router-link></li>
...
.menu li {
    ...
    position: relative;
    /* Add relative positioning for submenu */
}
...
.menu li a {
    display: block;
    width: 100%;
    /* Make the router-link fill the entire li area */
    color: inherit;
    font-weight: 600;
    ...
    /* Add a border between each link */
}
